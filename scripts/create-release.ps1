    ...
    foreach ($commit in $commitsRaw) {
        if (-not [string]::IsNullOrWhiteSpace($commit)) {
            ...
            if ($parts.Length -eq 4) {
                ...
                $author = $parts[2].Trim()
                $date = $parts[3].Trim()
                $formattedCommits += "- **$message** ([``$hash``](https://github.com/HueByte/Jiro.Libs/commit/$hash)) by @$author on $date"
            }
        }
    }
}
...
$changelogPath = "https://huebyte.github.io/Jiro.Libs/documentation/changelog/$Version.html"
...
**Full Changelog**: https://github.com/HueByte/Jiro.Libs/compare/$latestTag...$Version
"@
...
function Get-BuildArtifacts {
    ...
    if (-not [string]::IsNullOrWhiteSpace($CustomPath)) {
        ...
        # Jiro.Libs build output paths for .NET projects
        $searchPaths += @(
