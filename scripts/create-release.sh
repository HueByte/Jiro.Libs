...
if [ "$CURRENT_BRANCH" != "master" ]; then
    log_warning "⚠️ Warning: You are not on the master branch (current: $CURRENT_BRANCH)"
...
if [ -n "$COMMITS_RAW" ]; then
    while IFS='|' read -r hash message author date; do
        if [ -n "$hash" ] && [ -n "$message" ]; then
            FORMATTED_COMMITS="$FORMATTED_COMMITS- **$message** ([\`$hash\`](https://github.com/HueByte/Jiro.Libs/commit/$hash)) by @$author on $date
"
        fi
    done <<< "$COMMITS_RAW"
fi
...
CHANGELOG_PATH="https://huebyte.github.io/Jiro.Libs/documentation/changelog/$VERSION.html"
...
**Full Changelog**: https://github.com/HueByte/Jiro.Libs/compare/$LATEST_TAG...$VERSION"
